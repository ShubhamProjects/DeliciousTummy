...
export const colorStyles = Object.keys(palette).reduce((accumulator, name) => {
  const colorValue = palette[name];
  const capitalizedColorName = `${name.substr(0, 1).toUpperCase()}${name.substr(
    1,
  )}`;

  accumulator[name] = {color: colorValue};
  accumulator[`bg${capitalizedColorName}`] = {backgroundColor: colorValue};
  accumulator[`bc${capitalizedColorName}`] = {borderColor: colorValue};
